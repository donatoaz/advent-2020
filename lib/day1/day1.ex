defmodule Aoc2020.Day1 do
  ...
  defmodule Part1 do
    ...
    def run(input) do
      ...
      adds_to_2020 = Day1.build_complement_map(input, 2020)
  ...
  defmodule Part2 do
    ...
    def run(input) do
      ...
      adds_to_k =
        k
        |> Enum.map(fn e -> Day1.build_complement_map(input, e) end)

      {x, y} =
        adds_to_k
        |> Enum.map(fn e -> Day1.filter_list_from_map(input, e) end)
  ...
  def build_complement_map(list, n) do
    list |> Enum.map(fn e -> {n - e, e} end) |> Map.new()
  end

  def filter_list_from_map(list, map) do
